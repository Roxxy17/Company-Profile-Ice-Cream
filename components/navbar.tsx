...
import { Menu, X, IceCream, ShoppingCart } from "lucide-react"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <div className="flex items-center space-x-3">
              <Button variant="ghost" size="sm" className="relative p-2 hover:bg-emerald-50">
                <ShoppingCart className="h-5 w-5 text-gray-600" />
                <span className="absolute -top-1 -right-1 bg-emerald-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                  0
                </span>
              </Button>
              <Button className="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white px-6 py-2 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105">
                Pesan Sekarang
              </Button>
            </div>
          </div>
        ...
        {isOpen && (
          <div className="md:hidden animate-fade-in-up">
            <div className="px-4 pt-4 pb-6 space-y-2 bg-white/95 backdrop-blur-xl rounded-2xl mt-4 border border-emerald-100 shadow-xl">
              {navItems.map((item, index) => (
                <Link
                  key={item.name}
                  href={item.href}
